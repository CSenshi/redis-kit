...
describe('RedisLock', () => {
  describe('constructor', () => {
    ...
    it('should throw InvalidParameterError when Redis client is null', () => {
      expect(() => new RedisLock(null as any)).toThrow(InvalidParameterError);
      expect(() => new RedisLock(null as any)).toThrow();
    });

    it('should throw InvalidParameterError when Redis client is undefined', () => {
      expect(() => new RedisLock(undefined as any)).toThrow(InvalidParameterError);
      expect(() => new RedisLock(undefined as any)).toThrow();
    });

    it('should throw InvalidParameterError for invalid default TTL', () => {
      expect(() => new RedisLock(mockRedisClient, { defaultTtlMs: 0 }))
        .toThrow(InvalidParameterError);
      
      expect(() => new RedisLock(mockRedisClient, { defaultTtlMs: -1000 }))
        .toThrow(InvalidParameterError);
      
      expect(() => new RedisLock(mockRedisClient, { defaultTtlMs: 1.5 }))
        .toThrow(InvalidParameterError);
    });

    it('should throw InvalidParameterError for invalid token length', () => {
      expect(() => new RedisLock(mockRedisClient, { tokenLength: 0 }))
        .toThrow(InvalidParameterError);
      expect(() => new RedisLock(mockRedisClient, { tokenLength: 0 }))
        .toThrow('Invalid tokenLength: expected positive integer');
      
      expect(() => new RedisLock(mockRedisClient, { tokenLength: -5 }))
        .toThrow(InvalidParameterError);
      
      expect(() => new RedisLock(mockRedisClient, { tokenLength: 2.5 }))
        .toThrow(InvalidParameterError);
    });
  ...
  describe('acquire', () => {
    ...
    it('should successfully acquire a lock', async () => {
      ...
      expect(mockRedisClient.eval).toHaveBeenCalledWith(
        expect.stringContaining('redis.call("SET", KEYS[1], ARGV[1], "NX", "PX", ARGV[2])'),
        {
          keys: ['test-key'],
    ...
    it('should generate unique tokens for each acquisition', async () => {
      ...
      expect(result1.success && result1.token).not.toBe(result2.success && result2.token);
    });
    ...
    it('should throw InvalidParameterError for invalid key', async () => {
      await expect(lock.acquire('', 5000)).rejects.toThrow(InvalidParameterError);
      await expect(lock.acquire('', 5000)).rejects.toThrow('Invalid key: expected non-empty string');
      await expect(lock.acquire(null as any, 5000)).rejects.toThrow(InvalidParameterError);
      await expect(lock.acquire(undefined as any, 5000)).rejects.toThrow(InvalidParameterError);
      await expect(lock.acquire('   ', 5000)).rejects.toThrow(InvalidParameterError);
    });

    it('should throw InvalidParameterError for invalid TTL', async () => {
      await expect(lock.acquire('test-key', 0)).rejects.toThrow(InvalidParameterError);
      await expect(lock.acquire('test-key', 0)).rejects.toThrow('Invalid ttlMs: expected positive integer');
      await expect(lock.acquire('test-key', -1000)).rejects.toThrow(InvalidParameterError);
      await expect(lock.acquire('test-key', 1.5)).rejects.toThrow(InvalidParameterError);
    });
    ...
    it('should handle Redis connection errors', async () => {
      ...
      await expect(lock.acquire('test-key', 5000))
        .rejects.toThrow(RedisConnectionError);
      await expect(lock.acquire('test-key', 5000))
        .rejects.toThrow("Redis operation 'acquire' failed: Redis connection failed");
    });
    ...
    it('should handle unknown Redis errors', async () => {
      ...
      await expect(lock.acquire('test-key', 5000))
        .rejects.toThrow(RedisConnectionError);
      await expect(lock.acquire('test-key', 5000))
        .rejects.toThrow("Redis operation 'acquire' failed: Unknown error");
    });
  });
  ...
  describe('release', () => {
    ...
    it('should throw InvalidParameterError for invalid key', async () => {
      await expect(lock.release('', 'token')).rejects.toThrow(InvalidParameterError);
      await expect(lock.release('', 'token')).rejects.toThrow('Invalid key: expected non-empty string');
      await expect(lock.release(null as any, 'token')).rejects.toThrow(InvalidParameterError);
      await expect(lock.release(undefined as any, 'token')).rejects.toThrow(InvalidParameterError);
      await expect(lock.release('   ', 'token')).rejects.toThrow(InvalidParameterError);
    });

    it('should throw InvalidParameterError for invalid token', async () => {
      await expect(lock.release('test-key', '')).rejects.toThrow(InvalidParameterError);
      await expect(lock.release('test-key', '')).rejects.toThrow('Invalid token: expected non-empty string');
      await expect(lock.release('test-key', null as any)).rejects.toThrow(InvalidParameterError);
      await expect(lock.release('test-key', undefined as any)).rejects.toThrow(InvalidParameterError);
      await expect(lock.release('test-key', '   ')).rejects.toThrow(InvalidParameterError);
    });
    ...
    it('should handle Redis connection errors', async () => {
      ...
      await expect(lock.release('test-key', 'token'))
        .rejects.toThrow(RedisConnectionError);
      await expect(lock.release('test-key', 'token'))
        .rejects.toThrow("Redis operation 'release' failed: Redis connection failed");
    });
    ...
    it('should handle unknown Redis errors', async () => {
      ...
      await expect(lock.release('test-key', 'token'))
        .rejects.toThrow(RedisConnectionError);
      await expect(lock.release('test-key', 'token'))
        .rejects.toThrow("Redis operation 'release' failed: Unknown error");
    });
  });
  ...
  describe('extend', () => {
    ...
    it('should throw InvalidParameterError for invalid key', async () => {
      await expect(lock.extend('', 'token', 5000)).rejects.toThrow(InvalidParameterError);
      await expect(lock.extend('', 'token', 5000)).rejects.toThrow('Invalid key: expected non-empty string');
      await expect(lock.extend(null as any, 'token', 5000)).rejects.toThrow(InvalidParameterError);
      await expect(lock.extend(undefined as any, 'token', 5000)).rejects.toThrow(InvalidParameterError);
      await expect(lock.extend('   ', 'token', 5000)).rejects.toThrow(InvalidParameterError);
    });

    it('should throw InvalidParameterError for invalid token', async () => {
      await expect(lock.extend('test-key', '', 5000)).rejects.toThrow(InvalidParameterError);
      await expect(lock.extend('test-key', '', 5000)).rejects.toThrow('Invalid token: expected non-empty string');
      await expect(lock.extend('test-key', null as any, 5000)).rejects.toThrow(InvalidParameterError);
      await expect(lock.extend('test-key', undefined as any, 5000)).rejects.toThrow(InvalidParameterError);
      await expect(lock.extend('test-key', '   ', 5000)).rejects.toThrow(InvalidParameterError);
    });

    it('should throw InvalidParameterError for invalid TTL', async () => {
      await expect(lock.extend('test-key', 'token', 0)).rejects.toThrow(InvalidParameterError);
      await expect(lock.extend('test-key', 'token', 0)).rejects.toThrow('Invalid ttlMs: expected positive integer');
      await expect(lock.extend('test-key', 'token', -1000)).rejects.toThrow(InvalidParameterError);
      await expect(lock.extend('test-key', 'token', 1.5)).rejects.toThrow(InvalidParameterError);
    });
    ...
    it('should handle Redis connection errors', async () => {
      ...
      await expect(lock.extend('test-key', 'token', 5000))
        .rejects.toThrow(RedisConnectionError);
      await expect(lock.extend('test-key', 'token', 5000))
        .rejects.toThrow("Redis operation 'extend' failed: Redis connection failed");
    });
    ...
    it('should handle unknown Redis errors', async () => {
      ...
      await expect(lock.extend('test-key', 'token', 5000))
        .rejects.toThrow(RedisConnectionError);
      await expect(lock.extend('test-key', 'token', 5000))
        .rejects.toThrow("Redis operation 'extend' failed: Unknown error");
    });
  });
});
